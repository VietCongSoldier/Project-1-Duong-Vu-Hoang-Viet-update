...
namespace Project_1_Viet_9713.DAL
{
    public class HangHoaDAL : IHangHoaDAL
    {
        ...
        public List<HangHoa> LayDSSanPham()
        {
            ...
            while (read != null)
            {
                if (read != "")
                {
                    ...
                    list.Add(new HangHoa(plit[0],plit[1],plit[2], int.Parse(plit[3]), int.Parse(plit[4]), plit[5], int.Parse(plit[6])));
                }
                read = SR.ReadLine();
            }
            SR.Close();
        ...
        public void ThemSP(HangHoa hh)
        {
            ...
            SW.WriteLine($"{hh.Maloai}|{hh.Masp}|{hh.Tensp}|{hh.Dongia}|{hh.Soluong}|{hh.Donvitinh}|{hh.Thanhtien}|{hh.Ngaynhap}");
            SW.Close();
        }
        public void Update(List<HangHoa> list)
        {
            ...
            {
                SW.WriteLine($"{list[i].Maloai}|{list[i].Masp}|{list[i].Tensp}|{list[i].Dongia}|{list[i].Soluong}|{list[i].Donvitinh}|{list[i].Thanhtien}");
            }
            SW.Close();
        }
    }
